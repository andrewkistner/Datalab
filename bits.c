...
int bitXor(int x, int y) {
  //WE IMPLEMENT DE MORGAN'S LAW TO (~x & y) | (x & ~y)
  return ~(~x & ~y) & ~(x & y);
}
/*
...
int getByte(int x, int n) {
  /*n << 3 adjusts so that n is displayed in bits*/
  /*0xff is necessary to get least significant bit*/
  return (x >> (n << 3)) & 0xff;
}
/*
 ...
 */
int sign(int x) {
    /*shift 31 bits to most significant bit (sign bit)*/
    return ((!!x) | (x >> 31));
}
/*
...
int addOK(int x, int y) {
  ...
  /*c is sign of xy sum*/
  int c = z >> 31;
  /*overflow happens if sign of x and y are the same, but the sign of the sum of x and y is different*/
  return !!(a^b)|(!(a^c)&!(b^c));
}
...
unsigned floatNegate(unsigned uf) {
    /*change the sign bit of the value*/
    unsigned changeSign = (0x80000000 ^ uf);
    /*check the exponent bits*/
    unsigned exponentBits = 0xff << 23;
    /*if exponent bits are all 1's and frac bits have at least one bit, then it is NaN*/
    if(((exponentBits & uf) == exponentBits) && (uf & ((1 << 23) + (~0)))){
        return uf;
    }
    return changeSign;
}
...
int floatIsEqual(unsigned uf, unsigned ug) {
	unsigned numberMask = ~(1<<31);
	unsigned x = numberMask & uf;
	unsigned y = numberMask & ug;

	if ((x==0) && (y==0)){
	   return 1;
	}
	unsigned fractionMask = (~0) + (1<<23);

	if(((x>>23)==0xFF)&&((fractionMask & uf)!=0))||((y>>23)==0xFF)&&((fractionMask & ug)!=0))){
		return 0;
	}
